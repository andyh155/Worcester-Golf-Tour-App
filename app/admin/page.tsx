...
const brand = {
  ...
  data?.map((event: DbEventRow) => ({
    ...
    registration: event.registration_text || "Registration details coming soon",
...
function startEditingEvent(event: AppEvent) {
  ...
  setEditEventRegistrationText(
    event.registration === "Registration details coming soon" ? "" : event.registration
  );
  ...
  <div style={styles.card}>
    ...
    {pairings.map((item) => (
      ...
      >
        ...
        <div style={{ marginTop: 6, color: brand.muted }}>
          {events.find((e) => e.id === item.event_id)?.name || `Event #${item.event_id}`}
        </div>
        <div style={{ marginTop: 6, color: brand.muted }}>
          {item.tee_time}
